...
```{r grants}
grants <- readxl::read_xls("Grant_income.xls")
grants %>%
  group_by(Source) %>%
  summarise(
    Start = min(Start, na.rm=TRUE),
    End = max(End, na.rm=TRUE),
    #Research = ifelse(any(Research=="Y"), "Y", "N"),
    Amount = sum(Amount))


# Combine ESIPC, WesternPower, AEMO, Vencorp
#peakdemand <- grants %>%
#  filter_at(
#    vars(Project),
#    any_vars(grepl("ESIPC|Vencorp|Westernpower|AEMO",.)))
# Combine Tourism Australia grants
tourism <- grants %>%
  filter_at(
    vars(Source),
    any_vars(grepl("Tourism Australia",.)))
grants <- grants %>%
  filter_at(
    vars(Source),
    any_vars(!grepl("Tourism Australia",.))) %>%
  bind_rows(
#    summarise(peakdemand,
#      Source = "AEMO, ESPIC, Vencorp, Westernpower",
#      People = "RJ Hyndman, S Fan",
...
#      Amount = sum(Amount)),
    summarise(tourism,
      ...
      People = "RJ Hyndman, G Athanasopoulos",
      Title = "Tourism forecasting",
      Start = min(Start, na.rm=TRUE),
      End = max(End, na.rm=TRUE),
      Research = "Y",
      Amount = sum(Amount))) %>%
...
I have acquired (in most cases jointly) about $`r round(sum(grants[,"Amount"], na.rm=TRUE)/1e6,1)` million in external research grants since 2000. External research grants over $20,000 are listed below.\vspace*{0.2cm}

```{r grantstable, results='asis'}
grants %>%
  filter(Amount > 19999, Research=="Y") %>%
  mutate(
...
print_bibliography(pubs, "PhdThesis", "PhD thesis", startlabel = "papersstart")
print_bibliography(pubs, "Book", "Books")
# Normal articles excluding editorials
print_bibliography(pubs[!editorials], "Article", "Refereed research papers")
print_bibliography(pubs, c("InBook", "InCollection"), "Book chapters")
print_bibliography(pubs, "InProceedings", "Papers in refereed conference proceedings")
print_bibliography(pubs, c("TechReport", "Unpublished"), "Working papers under revision or review")
print_bibliography(pubs, "Review", "Book reviews")
# Now the editorials
print_bibliography(pubs[editorials], "Article", "Editorials", endlabel = "papersend")
print_bibliography(reports, "Report", "Statistical consulting reports",
  startlabel = "consultingstart", endlabel = "consultingend"
)
# Sort R packages by title, then year.
print_bibliography(packages, "Manual", "Software (R packages)",
  sorting = "ty",
