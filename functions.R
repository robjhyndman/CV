# A function that takes a Google Scholar ID, and returns a tibble
# with each row corresponding to a paper, and columns including
# authors, year, title, journal, adbc ranking, era ranking.
# The two rankings assign a letter to each journal (A*, A, B and C).

# In each case, produce a table showing number of papers of
# each class in each year.


prepare_fuzzy_Report <- function(user_ID) {
  ...
  df <- gcite_user_info(user = user_ID, secure = FALSE) %$% paper_df %>%
    select(authors, `publication date`, title, journal) %>%
    as_tibble() %>%
    set_colnames(c("Authors", "Year", "Title", "Journal")) %>%
    mutate(
  ...
  no_matchdf <- df %>%
    subset(Rank == "None") %>%
    mutate(
      Journal = as.character(fct_explicit_na(Journal, na_level = "999999999"))
    )
  ...
  bind_rows(df1, df2, df3) %>%
    mutate(Journal = ifelse((Journal == 999999999), NA, Journal)) %>%
    return()
}
...
# A function that takes a bib file as input and return a tibble containing
...
prepare_bib_report <- function(bib_file) {
  ...
  bind_rows(df1, df2, df3) %>%
    mutate(journal = ifelse((journal == 999999999), NA, journal)) %>%
    select(bibtype:year, journal, type, ADBC_ranking:Rank, institution, url:pages, doi:school) %>%
    return()
}
