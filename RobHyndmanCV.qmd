...
tribble(
  ~Start , ~End , ~Position                                                                                                                    ,
    ...
    2003 , 2026 , "\\textbf{Professor}, Department of Econometrics \\& Business Statistics, Monash University"                                 ,
    2016 , 2016 , "\\textbf{KNAW Visiting Professor}, TU/Eindhoven, Netherlands"                                                               ,
    2026 , NA   , "\\textbf{Vice-Chancellor's Distinguished Professor}, Department of Econometrics \\& Business Statistics, Monash University"
) |>
  mutate(
    order = is.na(End) * 1000 + Start,
...
- I currently supervise 2 PhD students.
  I have supervised to completion 34 PhD students and 3 Master's students.
...
- I have coauthored `r NROW(packages)` R packages as a result of my research (listed on pages \pageref{packagestart}--\pageref{packageend}).
  There have been over `r round(sum(rjh_packages$downloads, na.rm=TRUE)/1e6)` million downloads of my packages since 2015 (to `r format(Sys.time(), "%e %B %Y")`).

# Public lectures

- Ihaka Lecture, *Spotting the weird ones*, Auckland, Aug 2026.
...
- Keynote speaker, *Visualization of complex seasonal patterns in time series*, 800th anniversary, University of Padua, Sep 2022.
...
I have acquired (in most cases jointly) $`r sprintf("%2.1f", sumgrants/1e6)` million in external research grants since 2000.
External research grants over $`r prettyNum(max_grant, big.mark = ",", scientific = FALSE)` are listed below.
`\vspace*{0.2cm}\enlargethispage*{0.5cm}`{=latex}
...
grants |>
  filter(Amount > max_grant, Type == "Research") |>
  mutate(
...
- In 1998, I received the "Excellence in teaching" award from the Monash Science Society, and in 2020 I received the "Teaching Excellence" award from the Monash Student Association.
- With George Athanasopoulos, I have received the 2020 Dean's Award for Innovation in Learning and Teaching, the 2021 Vice-Chancellor's Award for Innovation in Learning and Teaching, and a 2022 Australian Awards for University Teaching citation for outstanding contributions to student learning.
- I am co-author, with George Athanasopoulos, of an innovative textbook entitled *Forecasting: principles and practice* ([`OTexts.com/fpp3/`](https://OTexts.com/fpp3)), which is available online and free of charge.
  The website has an average of over 25,000 pageviews per day.

# Consulting

- Since 1986, I have worked with several hundred different clients in Australia, the United States, Saudi Arabia, Israel, India, Russia, Switzerland, and the Netherlands.
  I have produced `r NROW(reports)` written consulting reports (listed on pages \pageref{consultingstart}--\pageref{consultingend}).
- Six times I have acted as an expert witness in litigation cases.

# Advisory boards

- Member of the Scaling Committee, Victorian Tertiary Admissions Centre (1994--2025).
  This committee is responsible for producing the ATAR for VCE students.
- Member of the ATAR Technical Group for the Australasian Conference of Tertiary Admissions Centres (2003--2026).
...
# Software (CRAN Task Views)

1. Hyndman, RJ, and R Killick (`r year`).
   CRAN Task View: Time Series Analysis.
   URL <https://cran.r-project.org/view=TimeSeries>
2. Talagala, TD, RJ Hyndman, and G Romano (`r year`).
