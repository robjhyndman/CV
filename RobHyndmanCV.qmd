...
# Get Google scholar stats
rjh <- gcite::gcite_citation_index(
  gcite::gcite_url("https://scholar.google.com.au/citations?user=vamErfkAAAAJ&hl=en")
)

# Update list of R packages I've coauthored
rjh_packages <- get_rjh_packages()

# Read any bib files and put in required order
pubs <- bibliography_entries("rjhpubs.bib") |>
  arrange(issued, author$family, title)
reports <- bibliography_entries("rjhreports.bib") |>
  arrange(issued, author$family, title)
packages <- bibliography_entries("Rpackages.bib") |>
  arrange(stringr::str_to_lower(title), issued)
...
# Research
  ...
  * I currently supervise five PhD students. I have previously supervised an additional 29 PhD students and 3 Masters students.
...
#| output: asis
pubs |>
  filter(type %in% c("manuscript", "report"), is.na(keyword)) |>
  add_bib_section()
```
...
pubs |>
  filter(type == "report", keyword == "misc") |>
  add_bib_section()
```
