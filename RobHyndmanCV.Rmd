...
grants <- grants %>%
  filter_at(
    vars(Project), 
    any_vars(!grepl("ESIPC|Vencorp|Westernpower|AEMO",.))) %>%
  bind_rows(summarise(peakdemand,
    Project = "AEMO, ESPIC, Vencorp, Westernpower",
    Start = min(Start),
...
I have acquired (in most cases jointly) about $`r sum(grants[,"Amount"], na.rm=TRUE)/1e6` million in external research grants since 2000. External research grants over $20,000 are listed below.\vspace*{0.2cm}
