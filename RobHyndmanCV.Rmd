...
# Research
  ...
  * I have coauthored `r NROW(packages)` R packages as a result of my research (listed on pages \pageref{packagestart}--\pageref{packageend}). There have been over `r round(sum(packages$downloads, na.rm=TRUE)/1e6)` million downloads of my packages since 2015 (to `r format(Sys.time(), "%e %B %Y")`).
...
```{r r-packages-list}
# Sort R packages by package name
packages
```
